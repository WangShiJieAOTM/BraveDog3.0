...
class YoLov5TRT(object):
    ...
    def infer(self, img):
        ...
        return image_raw, end - start, box
...
class inferThread(threading.Thread):
    ...
    def Detect(self):
        ...
        while(True):    
            ...
            self.readOk = False
            self.con.release()
            image_raw, use_time, self.result_box = self.yolov5_wrapper.infer(image)
            cv2.imshow("111", image_raw)
            cv2.waitKey(1)
    def Judge(self):
        #judge
        if (len(self.result_box) > 0):
            machine_conf.judge(self.result_box)
            if DEBUG:
                cv2.imshow("result", image_raw)
        else:
            print("none")
    ...
    def run(self):
        ...
        #t3 = threading.Thread(target = self.Judge)
        t1.setDaemon(True)
        t2.setDaemon(True)
        t1.start()
        t2.start()
