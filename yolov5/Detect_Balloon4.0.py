...
class YoLov5TRT(object):
    ...
    def infer(self, img):
        ...
        box = []
        if box_num != 0:
            max_scores = result_scores[0]
            max_scores_index = 0
            for i in range(box_num):
                if result_scores[i] > max_scores:
                    max_scores = result_scores[i]
                    max_scores_index = i
            box = result_boxes[max_scores_index]
            plot_one_box(
                box,
                image_raw,
                label="{}:{:.2f}".format(
                categories[int(result_classid[max_scores_index])],
                result_scores[max_scores_index],
                ),
                color = [0,255,0]
            )
        # cv2.imshow("result", image_raw)
        return box_num != 0, image_raw, end - start, box
...
class MachineConf:
    ...
    def decodeDisplay(self, image):
        ...
        if(len(barcodes) == 0):
            ...
            for i in range(self.width//2-20, self.width//2+20):               #center point     
            ...
            if num > 0:
                ...
                if(center_depth_area_mean < 0.35):                             #need to change
                    barcodeData = 'go'
    ...
    def judge(self, result_box):      #judgement
        box_center_x, box_center_y = get_box_center(result_box)
        ...
        num = 0
        target_depth = 0
        for i in range(max(0,int(box_center_x) - 20), min(int(box_center_x) + 20, self.width)):
            for j in range(max(0,int(box_center_y) - 20), min(int(box_center_y) + 20, self.height)):
                 if self.aligned_depth_frame.get_distance(i, j) > 0:
                     target_depth += self.aligned_depth_frame.get_distance(i, j)
                     num += 1
        if num > 0:
                target_depth =  target_depth / num
        target_xy_true = [(box_center_x - ppx) * target_depth / fx,
                              (box_center_y - ppy) * target_depth / fy]
        if(target_depth > 0):
            angle_yaw = np.arctan((target_xy_true[0]-0.0035) / (target_depth + 0.02525)) / 2 / math.pi * 360           

        if box_center_x > image_center_x + DEAD_AREA:    # = 0  > 2 
            send_data = 'left'
        elif box_center_x < image_center_x - DEAD_AREA:
            send_data = 'right'
        else:
            send_data = 'mid'
        
        if (target_depth < 1.5 and target_depth > 0.5):
            send_data = str(angle_yaw)
        if (target_depth < 0.5 and target_depth > 0): 
            #print(target_depth)
...
class inferThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            detected_flag, image_raw, use_time, result_box = self.yolov5_wrapper.infer(self.color_img)
            #print('Time: ', use_time)
            if detected_flag:
                machine_conf.judge(result_box)
                if(DEBUG):
                    cv2.imshow("result", image_raw)
            else:
                decode_img = machine_conf.decodeDisplay(self.color_img)
                if(DEBUG):
                    cv2.imshow("result", decode_img)
            if cv2.waitKey(1) == 27:
