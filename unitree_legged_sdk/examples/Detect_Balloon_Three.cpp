...
float pid_para[3] = {4.0f, 0.0f, 0.1f};
float max_out = 1.0f;
float max_iout = 0.0f;
...
float adjust_translation_v = -0.25f;			//	接收识别调整时平移速度
float go_v = 0.4f;							//	识别结束自主前进时速度
...
#define BACK_TIME 0.8 * TIME_RATIO	//	识别结束自主后退时间
...
public:
	...
	void turn_angle(float err);  //旋转调整角度
	void move_angle(float err);  //平移调整角度
...
void Custom::move_angle(float err) //狗子运动第二阶段 找红色气球的姿态角度偏转
{
	...
	float temp_x_speed = PID_calc(&yaw_angle_pid, measure_angle, target_angle);

	std::cout << "measure_angle:" << measure_angle
			  << " target_angle:" << target_angle
			  << " PID_calc:" << temp_x_speed << std::endl;

	cmd.mode = 2;
	std:cout<<err<<std::endl;
	//cmd.velocity[0] = 0.2f;
	cmd.velocity[1] = -temp_x_speed;
}
